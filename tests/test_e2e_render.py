...
from musipelago.utils import filter_to_ascii, filter_py_json

TPL = os.path.join(os.path.dirname(g.__file__), "apworld_template")

TEMPLATES = ["Locations.py.j2", "Items.py.j2", "Options.py.j2", "Types.py.j2",
             "Regions.py.j2", "Rules.py.j2", "__init__.py.j2"]


def _track(uri, title, artist):
    return GenericTrack(uri=uri, title=title, artist=artist, album_title="o",
                        duration_ms=1000, service="local")


def _album(uri, n, artist):
    # deliberately reuse track titles across albums so collisions are likely
    tr = [_track(f"{uri}/{i}", ["Intro", "Outro", "Jam", "Intro"][i % 4], artist)
          for i in range(n)]
