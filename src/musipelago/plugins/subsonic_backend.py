...
class SubsonicLoginUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.add_widget(
            Label(
                text="Server URL (e.g. http://music.server.com):",
                halign="left",
                size_hint_y=None,
                height=dp(30),
            )
        ...
        self.username_input = TextInput(
            hint_text="user", multiline=False, write_tab=False, size_hint_y=None, height=dp(40)
        )
...
class SubsonicBackendLogic(AbstractMusicBackend):
    ...
    def _auth_thread(self, server, user, pwd):
        try:
            ...
            msg = str(e)
            Logger.error(f"Subsonic Login Error: {msg}")
    ...
    def initialize_client(self, client_data, app):
        ...
        if json_server:
            json_server = json_server.rstrip("/")
    ...
    def _sign_url(self, fragment):
        if not fragment or not fragment.startswith("coverArt:"):
            return ""
...
class SubsonicHostUI(AbstractPluginHost):
    ...
    def on_item_menu_click(self, list_id, item):
        return False
...
class SubsonicSettingsWidget(BoxLayout):
    def __init__(self, host_instance, **kwargs):
        ...
        self.format_spinner = Spinner(
            text=self.host.transcode_format, values=("mp3", "raw"), size_hint_x=0.6
        )
...
class SubsonicClientHost(AbstractClientHost):
    ...
    def on_list_item_click(self, item):
        if item.raw_item_type == "album":
            ...
            self.root_layout.populate_track_list(
                item.raw_uri, local_image_path=album.display_image_url
            )
    ...
    def start_polling(self):
        if not self.poll_event:
            self.poll_event = Clock.schedule_interval(self._update_progress, 0.1)
                ...
                if not data["is_finished"]:
                    ...
                    if lid := data.get("location_id"):
                        if self.app.ap_client:
                            self.app.ap_client.send_location_check(lid)
    ...
    def on_menu_action(self, txt, item):
        if txt == "OPEN_MENU":
            ...
            if self.app.cheat_mode:
                cheat_text = "[color=ff8888]Send Location[/color]"
